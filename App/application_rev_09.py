...
class MainFrame(wx.Frame):
    def __init__(self):
        ...
        # Panel search (phía trên)
        panel_search = wx.Panel(content_panel)
        panel_search.SetBackgroundColour("#B1D4E0")
        panel_search_sizer = wx.BoxSizer(wx.HORIZONTAL)

        # Panel find (trái)
        panel_find = wx.Panel(panel_search)
        panel_find.SetBackgroundColour("#B1D4E0")
        panel_find_sizer = wx.BoxSizer(wx.VERTICAL)

        # Hàm tạo search box kết hợp dropdown
        def create_search_box(parent, label, search_fields, size=(300, -1)):
            sizer = wx.BoxSizer(wx.HORIZONTAL)
            search_label = wx.StaticText(parent, label=label)
            search_box = wx.SearchCtrl(parent, style=wx.TE_PROCESS_ENTER, size=size)
            search_box.ShowSearchButton(True)
            search_box.ShowCancelButton(True)
            search_menu = wx.Menu()
            for idx, field in enumerate(search_fields):
                item = search_menu.AppendRadioItem(1000 + idx, field)
                if idx == 0:
                    item.Check(True)
            search_box.SetMenu(search_menu)
            sizer.Add(search_label, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 8)
            sizer.Add(search_box, 1, wx.EXPAND)
            return sizer

        search_fields = [
            "System No", "System Description", "SubSystem No", "SubSystem Description", "Tag No", "Tag Description",
            "Discipline Code", "Checksheet Type", "Status", "Complete Date", "File Upload", "Plan Start", "Plan Finish",
            "Norm", "Upload by"
        ]

        # System và Subsystem ở phía trên
        panel_find_sizer.Add(create_search_box(panel_find, "System:", search_fields), 0, wx.EXPAND | wx.ALL, 2)
        panel_find_sizer.Add(create_search_box(panel_find, "Subsystem:", search_fields), 0, wx.EXPAND | wx.ALL, 2)
        # Discipline và Phase ở phía dưới
        panel_find_sizer.Add(create_search_box(panel_find, "Discipline:", search_fields), 0, wx.EXPAND | wx.ALL, 2)
        panel_find_sizer.Add(create_search_box(panel_find, "Phase:", search_fields), 0, wx.EXPAND | wx.ALL, 2)
        panel_find.SetSizer(panel_find_sizer)

        # Panel button (phải)
        panel_button = wx.Panel(panel_search)
        panel_button.SetBackgroundColour("#B1D4E0")
        panel_button_sizer = wx.BoxSizer(wx.VERTICAL)
        btn_load = wx.Button(panel_button, label="📂 Load", size=(120, 40))
        panel_button_sizer.AddStretchSpacer()
        panel_button_sizer.Add(btn_load, 0, wx.ALIGN_RIGHT | wx.ALL, 4)
        panel_button_sizer.AddStretchSpacer()
        panel_button.SetSizer(panel_button_sizer)

        # Add panel_find (trái) và panel_button (phải) vào panel_search_sizer
        panel_search_sizer.Add(panel_find, 1, wx.EXPAND | wx.ALL, 0)
        panel_search_sizer.Add(panel_button, 0, wx.EXPAND | wx.LEFT, 8)
        panel_search.SetSizer(panel_search_sizer)
        content_sizer.Add(panel_search, 0, wx.EXPAND | wx.ALL, 4)

        # Panel display_data (phía dưới)
        panel_display = wx.Panel(content_panel)
        panel_display.SetBackgroundColour("#2E8BC0")
        display_sizer = wx.BoxSizer(wx.VERTICAL)

        self.grid = gridlib.Grid(panel_display)
        col_labels = [
            "System No", "System Description", "SubSystem No", "SubSystem Description", "Tag No", "Tag Description",
            "Discipline Code", "Checksheet Type", "Status", "Complete Date", "File Upload", "Plan Start", "Plan Finish",
            "Norm", "Upload by"
        ...
        display_sizer.Add(self.grid, 1, wx.EXPAND | wx.ALL, 8)

        log_box = wx.StaticBox(panel_display, label="Logs")
        log_sizer = wx.StaticBoxSizer(log_box, wx.VERTICAL)
        self.log_ctrl = wx.TextCtrl(panel_display, style=wx.TE_MULTILINE | wx.TE_READONLY, size=(-1, 120))
        log_sizer.Add(self.log_ctrl, 1, wx.EXPAND | wx.ALL, 5)
        display_sizer.Add(log_sizer, 0, wx.EXPAND | wx.ALL, 8)

        panel_display.SetSizer(display_sizer)
        content_sizer.Add(panel_display, 1, wx.EXPAND | wx.ALL, 0)
